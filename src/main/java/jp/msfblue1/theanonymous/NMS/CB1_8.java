 ...
 */
public class CB1_8 implements NMSAdapter {
    @Override
    public void setName(Player p) {

        String ename = encryptName(p.getAddress().getAddress().getHostAddress()+new SimpleDateFormat("yyyy/MM/dd").format(new Date()));

        CraftPlayer cp = (CraftPlayer) p;
        EntityHuman eh = cp.getHandle();
        GameProfile gp = new GameProfile(cp.getUniqueId(),ename);
        try {
            setObjectFINALPRVATE(eh,"bF",gp);

            for (Player other : Bukkit.getOnlinePlayers()){
                if(!p.equals(other)){
                    EntityPlayer oep = ((CraftPlayer)p).getHandle();
    ...
    public void sendNavTitle(Player p, String Mes) {
        PacketPlayOutChat playOutChat = new PacketPlayOutChat(ChatSerializer.a("{\"text\": \"" + Mes + "\"}"));
        getEntityPlayer(p).playerConnection.sendPacket(playOutChat);
    }

    public static void setObjectFINALPRVATE(EntityHuman obj, String name, Object value) {
        Field f;
        try {
    ...
    private void updateSkin(Player player){
        ...
        try{
            ...
            PacketPlayOutPlayerInfo removeinfo = new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER,itr);
            ...
            PacketPlayOutPlayerInfo addinfo = new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER,itr);
            ...
            PacketPlayOutEntityEquipment hand = new PacketPlayOutEntityEquipment(player.getEntityId(),0, CraftItemStack.asNMSCopy(player.getItemInHand()));

            //PacketPlayOutEntityEquipment offhand = new PacketPlayOutEntityEquipment(player.getEntityId(),4,CraftItemStack.asNMSCopy(player.getItemInHand()));

            PacketPlayOutEntityEquipment helmet = new PacketPlayOutEntityEquipment(player.getEntityId(),4,CraftItemStack.asNMSCopy(player.getItemInHand()));

            PacketPlayOutEntityEquipment chestplate = new PacketPlayOutEntityEquipment(player.getEntityId(),3,CraftItemStack.asNMSCopy(player.getItemInHand()));

            PacketPlayOutEntityEquipment leggings = new PacketPlayOutEntityEquipment(player.getEntityId(),2,CraftItemStack.asNMSCopy(player.getItemInHand()));

            PacketPlayOutEntityEquipment boots = new PacketPlayOutEntityEquipment(player.getEntityId(),1,CraftItemStack.asNMSCopy(player.getItemInHand()));
